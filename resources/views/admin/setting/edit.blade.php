...
<div class="container-fluid pt-4 px-4">
    ...
    @endif
    {!! Form::open(['url' => ['admin/setting/store'.'/'.$id], 'class' => ['card__form'], 'files' => true]) !!}
        @csrf
        <div class="row g-4">
            <div class="col-sm-12 col-xl-6">
                <div class="bg-secondary rounded h-100 p-4">
                    <h6 class="mb-4">Cập nhật thiết lập</h6>
                    <div class="mb-3">
                        {!! Form::label('zalo', 'Zalo:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::text('zalo', $zalo, ['class'=>['card__name', 'card__zalo', 'form-control'] ,'placeholder'=>'Zalo']) !!}
                    </div>
                    <div class="mb-3">
                        {!! Form::label('call', 'Điện thoại:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::number('call', $call, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Điện thoại']) !!}
                    </div>
                    <div class="mb-3">
                        {!! Form::label('hotline', 'Liên hệ:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::number('hotline', $hotline, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Liên hệ']) !!}
                    </div>
                    <div class="mb-3">
                        {!! Form::label('messager', 'Messager', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::text('messager', $messager, ['class'=>['card__name', 'card__messager', 'form-control'] ,'placeholder'=>'Messager']) !!}
                    </div>
                    <div class="mb-3">
                        {!! Form::label('map', 'Bản đồ:', ['class'=>['card__label--desc','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::textarea('map', $map, ['class' => ['form-control','card__desc'], 'placeholder' => 'Bản đồ']) !!}
                    </div>
                    <div class="mb-3">
                        {!! Form::label('fanpage', 'Fanpage:', ['class'=>['card__label--desc','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::textarea('fanpage', $fanpage, ['class' => ['form-control','card__desc'], 'placeholder' => 'Fanpage']) !!}
                    </div>
            ...
            <div class="col-sm-12 col-xl-6 bg-secondary">
                <div class="mb-3">
                    ...
                    {!! Form::text('name', $name, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Tên công ty']) !!}
                </div>
                <div class="mb-3">
                    {!! Form::label('worktime', 'Thời gian làm việc:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                    {!! Form::text('worktime', $worktime, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Thời gian làm việc']) !!}
                </div>
                <div class="mb-3">
                    {!! Form::label('address', 'Địa chỉ:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                    {!! Form::text('address', $address, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Địa chỉ']) !!}
                </div>
                <div class="mb-3">
                    {!! Form::label('copyright', 'Copyright:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                    {!! Form::text('copyright', $copyright, ['class'=>['card__name', 'card__call', 'form-control'] ,'placeholder'=>'Copyright']) !!}
                </div>
            </div>
        </div>
    {!! Form::close() !!}
