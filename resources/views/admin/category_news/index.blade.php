...
<div class="container-fluid pt-4 px-4">
    ...
    <div class="bg-secondary rounded h-100 p-4">
        <h6 class="mb-4 uppercase text-center text-[24px]">Danh sách danh mục tin tức</h6>
        {!! Form::open(['url' => ['admin/category_news/action'], 'class' => ['card__body']]) !!}
            @csrf
            <div class="card__body--analytic mb-[12px]">
                <a href="{{ request()->fullUrlWithQuery(['status' => 'active']) }}" class="card__body--active text-blue-600 mr-[8px]">Đang hoạt động ({{ $countCategoryNews[0] }})</a>
                <a href="{{ request()->fullUrlWithQuery(['status' => 'trash']) }}" class="card__body--trash text-red-600">Vô hiệu hóa ({{ $countCategoryNews[1] }})</a>
            </div>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    @if ($categoryNews->total() > 0)
                        @php
                            $k = 0;
                        @endphp

                        @foreach ($categoryNews as $k => $v)    
                            @php
                                $k++;
                            @endphp
                            <tr>
                                ...
                                <td>
                                    @if ($v->photo)
                                        <a href="{{ route('admin.category_news.edit', $v->id) }}">
                                ...
                                <td>
                                    @if ($v->level == 1)
                                        <a href="{{ route('admin.category_news.edit', $v->id) }}">{{ $v->name }}</a>
                                    @endif
                                    @if ($v->level == 2)
                                        <a href="{{ route('admin.category_news2.edit', $v->id) }}">{{ $v->name }}</a>
                                    @endif
                                </td>
                                ...
                                <td>@if ($v->status == 'trash')
                                    <span></span>
                                @else
                                    <input type="checkbox" class="custom-control-input form-check-input show-checkbox-catNew remove-checkbox-catNew" data-id="{{ $v->id }}" data-show="noibat" {{ $v->state == 'noibat' ? 'checked' : ''}}>
                                @endif</td>
                                ...
                                @if ($v->status != 'trash')
                                    <td>
                                        <a href="@if ($v->level == 1)
                                            {!! route('admin.category_news.edit', $v->id) !!}
                                        @else
                                            {!! route('admin.category_news2.edit', $v->id) !!}
                                        @endif" class="text-lime-600 m-[12px]"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <a href="#" data-id="{{ $v->id }}" class="delete__catNew text-red-700"><i class="fa-solid fa-trash pointer-events-none"></i></a>
                                    </td>
                                @endif
                            </tr>
                        @endforeach
                    @else
                        <tr><td colspan="12"><span class="text-red-600 block p-[12px]">Không có danh mục tin tức</span></td></tr>
                    @endif
                </tbody>
            </table>
        {!! Form::close() !!}
        {!! Form::open(['name' => 'delete__form', 'class' => ['card__body']]) !!}
            ...
            @method('DELETE')
        {!! Form::close() !!}
        {!! $categoryNews->links("admin.layout.pagination") !!}
    </div>
</div>
@endsection