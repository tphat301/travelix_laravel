...
<div class="container-fluid pt-4 px-4">
    ...
    @endif
    {!! Form::open(['url' => ['admin/slideshow/store'.'/'], 'class' => ['card__form'], 'files' => true]) !!}
        @csrf
        <div class="row g-4">
            <div class="col-sm-12 col-xl-6">
                <div class="bg-secondary rounded h-100 p-4">
                    <h6 class="mb-4">Thêm {{ __('Slideshow') }}</h6>
                    <div class="mb-3">
                        {!! Form::label('name', 'Tiêu đề:', ['class'=>['card__label--name','col-sm-4', 'col-form-label']]); !!}
                        {!! Form::text('name', '', ['class'=>['card__name', 'name__slideshow', 'form-control'] ,'placeholder'=>'Tiêu đề']) !!}
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        {!! Form::text('link', '', ['class'=>['card__name', 'name__slideshow', 'form-control'] ,'placeholder'=>'Link']) !!}
                        @error('name')
                            <small class="text-red-700">{{ $message }}</small>
                        @enderror
                    </div>
