...
<div class="container-fluid pt-4 px-4">
    ...
    <div class="bg-secondary rounded h-100 p-4">
        <h6 class="mb-4 uppercase text-center text-[24px]">Danh sách dịch vụ</h6>
        {!! Form::open(['url' => ['admin/service/action'], 'class' => ['card__body']]) !!}
            @csrf
            <div class="card__body--analytic mb-[12px]">
                <a href="{{ request()->fullUrlWithQuery(['status' => 'active']) }}" class="card__body--active text-blue-600 mr-[8px]">Đang hoạt động ({{ $countService[0] }})</a>
                <a href="{{ request()->fullUrlWithQuery(['status' => 'trash']) }}" class="card__body--trash text-red-600">Vô hiệu hóa ({{ $countService[1] }})</a>
            </div>
                    ...
                    @if ($services->total() > 0)
                        ...
                        @foreach ($services as $k => $v)    
                            ...
                            <tr>
                                ...
                                <td><a href="{{ route('admin.service.edit', $v->id) }}">{{ $v->name }}</a></td>
                                <td><span class="text-danger">{{ number_format($v->price, 0, ",", ".") }}VND</span></td>
                                <td><span class="text-warning">{{ $v->discount }}%</span></td>
                                <td><span>{{ $v->brand }}</span></td>
                                <td>
                                    <input type="checkbox" class="custom-control-input form-check-input show-checkbox remove-checkbox" data-id="{{ $v->id }}" data-show="noibat" {{ $v->state == 'noibat' ? 'checked' : ''}}>
                                </td>
                                ...
                                @if ($v->status != 'trash')
                                    <td>
                                        <a href="{{ route('admin.service.copy', $v->id) }}" class="text-lime-600 m-[12px]"><i class="fa-solid fa-copy"></i></i></a>
        ...
        {!! $services->links("admin.layout.pagination") !!}
    </div>
</div>
@endsection